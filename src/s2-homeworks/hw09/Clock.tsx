...
function Clock() {
    ...
    const start = () => {
        setTimerId(+setInterval(() => {
            ...
            // let dateNow = restoreState('hw9-date',  date)
            setDate(new Date())
        }, 1000))
    ...
    const stringTime =
        `${formatterTime.format(date)}` || <br/> // часы24:минуты:секунды (01:02:03)/(23:02:03)/(24:00:00)/(00:00:01) // пишут студенты

    const formatterDate = new Intl.DateTimeFormat("ru",)
    const stringDate =
        `${formatterDate.format(date)}` || <br/> // день.месяц.год (01.02.2022) // пишут студенты, варианты 01.02.0123/01.02.-123/01.02.12345 не рассматриваем

    // день недели на английском, месяц на английском (https://learn.javascript.ru/intl#intl-datetimeformat)
    const formatterDay = new Intl.DateTimeFormat("en-US", {
        weekday: 'long'
    })
    const stringDay: string | JSX.Element | Date = `${formatterDay.format(date)}` || <br/> // пишут студенты
